...
const PlaceDetail = ({ recordId }: { recordId: string }) => {
  return (
    <>
      <div className="flex flex-col  shadow-lg border-1 w-full h-full bg-gray-100 dark:bg-gray-800 mx-auto overflow-hidden">
        <CloseButton onClick={() => window.location.reload()} />
        <div className="img-container w-full bg-red-700 h-2/6 ">
          <img
        ...
        >
          ...
          <ul className="space-y-2 ">
            {Array.from({ length: 15 }).map((_, i) => (
              <li key={i}>
                <p className="text-sm leading-6 font-semibold text-zinc-700 dark:text-zinc-300">
                  Vavaʻu, Tonga
                </p>
                <p className="text-sm leading-6 font-normal text-zinc-500 dark:text-zinc-100">
